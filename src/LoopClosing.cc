...
namespace ORB_SLAM3 {
  ...
  bool LoopClosing::NewDetectCommonRegions() {
    ...
    if (!bLoopDetectedInKF && !vpLoopBowCand.empty()) {
      mbLoopDetected = DetectCommonRegionsFromBoW(
          vpLoopBowCand, mpLoopMatchedKF, mpLoopLastCurrentKF, mg2oLoopSlw, mnLoopNumCoincidences,
  ...
  bool LoopClosing::DetectAndReffineSim3FromLastKF(KeyFrame* pCurrentKF, KeyFrame* pMatchedKF,
    ...
    nNumProjMatches = FindMatchesByProjection(pCurrentKF, pMatchedKF, gScw, spAlreadyMatchedMPs,
                                              vpMPs, vpMatchedMPs);
    ...
    if (nNumProjMatches >= nProjMatches) {
      // Verbose::PrintMess("Sim3 reffine: There are " + to_string(nNumProjMatches) + " initial
      ...
      if (mpTracker->mSensor == System::IMU_MONOCULAR && !pCurrentKF->GetMap()->GetIniertialBA2())
        bFixedScale = false;
      int numOptMatches = Optimizer::OptimizeSim3(mpCurrentKF, pMatchedKF, vpMatchedMPs, gScm, 10,
                                                  bFixedScale, mHessian7x7, true);

      // Verbose::PrintMess("Sim3 reffine: There are " + to_string(numOptMatches) + " matches after
      // of the optimization ", Verbose::VERBOSITY_DEBUG);
  ...
  bool LoopClosing::DetectCommonRegionsFromBoW(std::vector<KeyFrame*>& vpBowCand,
    ...
    int index = 0;
    // Verbose::PrintMess("BoW candidates: There are " + to_string(vpBowCand.size()) + " possible
    // candidates ", Verbose::VERBOSITY_DEBUG);
    for (KeyFrame* pKFi : vpBowCand) {
      ...
      if (numBoWMatches >= nBoWMatches)  // TODO pick a good threshold
      {
        ...
        while (!bConverge && !bNoMore) {
          ...
          // Verbose::PrintMess("BoW guess: Solver achieve " + to_string(nInliers) + " geometrical
          // inliers among " + to_string(nBoWInliers) + " BoW matches", Verbose::VERBOSITY_DEBUG);
        }

        if (bConverge) {
          // std::cout << "Check BoW: SolverSim3 converged" << std::endl;

          // Verbose::PrintMess("BoW guess: Convergende with " + to_string(nInliers) + " geometrical
          // inliers among " + to_string(nBoWInliers) + " BoW matches", Verbose::VERBOSITY_DEBUG);
  ...
  void LoopClosing::CorrectLoop() {
    ...
    // std::cout << "Loop: number of connected KFs -> " + to_string(mvpCurrentConnectedKFs.size())
    // << std::endl;
...

    {
      ...
      for (vector<KeyFrame*>::iterator vit = mvpCurrentConnectedKFs.begin(),
        ...
        if (pKFi != mpCurrentKF) {
          ...
          g2o::Sim3 g2oSiw(Tiw.unit_quaternion().cast<double>(), Tiw.translation().cast<double>(),
                           1.0);
    ...
    // Launch a new thread to perform Global Bundle Adjustment (Only if few keyframes, if not it
    ...
    if (!pLoopMap->isImuInitialized()
      ...
      mpThreadGBA
          = new thread(&LoopClosing::RunGlobalBundleAdjustment, this, pLoopMap, mpCurrentKF->mnId);
    }
  ...
  void LoopClosing::MergeLocal() {
    ...
    // to_string(spLocalWindowKFs.size()) << "; #MPs = " << to_string(spLocalWindowMPs.size()) <<
    // std::endl;
    ...
    // to_string(spMergeConnectedKFs.size()) << "; #MPs = " << to_string(spMapPointMerge.size()) <<
    // std::endl;
    ...
    {
        std::cout << "[Merge]: " << std::to_string(numPointsWithCorrection) << " points removed from
    Ma due to its reference KF is not in welding area" << std::endl; std::cout << "[Merge]: Ma has "
...

    {
      ...
      unique_lock<mutex> mergeLock(
          pMergeMap
              ->mMutexMapUpdate);  // We remove the Kfs and MPs in the merged area from the old map
    ...
    if (mpTracker->mSensor == System::IMU_MONOCULAR || mpTracker->mSensor == System::IMU_STEREO
        || mpTracker->mSensor == System::IMU_RGBD) {
      Optimizer::MergeInertialBA(mpCurrentKF, mpMergeMatchedKF, &bStop, pCurrentMap,
                                 vCorrectedSim3);
    } else {
    ...
    if (vpCurrentMapKFs.size() == 0) {
...

      {
        ...
        unique_lock<mutex> currentLock(
            pCurrentMap->mMutexMapUpdate);  // We update the current map with the Merge information
        unique_lock<mutex> mergeLock(
            pMergeMap->mMutexMapUpdate);  // We remove the Kfs and MPs in the merged area from the
                                          // old map
    ...
    if (bRelaunchBA
      ...
      mbStopGBA = false;
      mpThreadGBA
          = new thread(&LoopClosing::RunGlobalBundleAdjustment, this, pMergeMap, mpCurrentKF->mnId);
    }
  ...
  void LoopClosing::MergeLocal2() {
    ...
    pNewParent = mpMergeMatchedKF;  // Old child, now it will be the parent of its own parent(we
    ...
    // TODO Check: If new map is too small, we suppose that not informaiton can be propagated from
    // new to old map
    if (numKFnew < 10) {
      mpLocalMapper->Release();
      return;
  ...
  void LoopClosing::CheckObservations(set<KeyFrame*>& spKFsMap1, set<KeyFrame*>& spKFsMap2) {
    ...
    for (KeyFrame* pKFi1 : spKFsMap1) {
      ...
      if (mMatchedMP.size() == 0) {
        cout << "CHECK-OBS: KF " << pKFi1->mnId << " has not any matched MP with the other map"
             << endl;
      } else {
        ...
        for (pair<KeyFrame*, int> matchedKF : mMatchedMP) {
          cout << "   -KF: " << matchedKF.first->mnId << ", Number of matches: " << matchedKF.second
               << endl;
        }
      }
    }
    ...
    // We need to propagate the correction through the spanning tree
    {
      ...
      if (!mbStopGBA) {
        ...
        while (!lpKFtoCheck.empty()) {
          ...
          // cout << "---Updating KF " << pKF->mnId << " with " << sChilds.size() << " childs" <<
          // endl; cout << " KF mnBAGlobalForKF: " << pKF->mnBAGlobalForKF << endl;
          ...
          for (set<KeyFrame*>::const_iterator sit = sChilds.begin(); sit != sChilds.end(); sit++) {
            ...
            if (pChild->mnBAGlobalForKF != nLoopKF) {
              ...
              // nLoopKF << endl; cout << " child id: " << pChild->mnId << endl;
          ...
          pKF->GetConnectedKeyFrames().size() << " connected KFs" << endl; cout << "--It has " <<
          pKF->GetCovisiblesByWeight(80).size() << " connected KF with 80 common matches or more" <<
          endl; cout << "--It has " << pKF->GetCovisiblesByWeight(50).size() << " connected KF with
          50 common matches or more" << endl; cout << "--It has " <<
          pKF->GetCovisiblesByWeight(20).size() << " connected KF with 20 common matches or more" <<
          endl;

              cout << "--STD in meters(x, y, z): " << desvX << ", " << desvY << ", " << desvZ <<
          endl;
          ...
          to_string(pKF->mnId) +"_D" + to_string(dist) +".png"; cv::imwrite(namefile, imLeft);
          }*/
