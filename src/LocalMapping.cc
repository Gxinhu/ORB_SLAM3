...
namespace ORB_SLAM3 {
  ...
  void LocalMapping::InitializeIMU(float priorG, float priorA, bool bFIBA) {
    ...
    Optimizer::InertialOptimization(mpAtlas->GetCurrentMap(), mRwg, mScale, mbg, mba, mbMonocular,
    ...
    if (bFIBA) {
      if (priorA != 0.f)
        Optimizer::FullInertialBA(mpAtlas->GetCurrentMap(), 100, false, mpCurrentKeyFrame->mnId,
  ...
  void LocalMapping::ScaleRefinement() {
    ...
    // difference between monocular and stereo unique_lock<mutex> lock0(mMutexImuInit);
    if (mbResetRequested) return;
