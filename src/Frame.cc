...
namespace ORB_SLAM3 {
  ...
  Frame::Frame(const cv::Mat &imGray, const double &timeStamp, ORBextractor *extractor,
    ...
    mmProjectPoints
        .clear();  // = map<long unsigned int, cv::Point2f>(N, static_cast<cv::Point2f>(NULL));
    mmMatchedInImage.clear();
  ...
  void Frame::SetImuPoseVelocity(const Eigen::Matrix3f &Rwb, const Eigen::Vector3f &twb,
  ...
  void Frame::UndistortKeyPoints() {
    ...
    // Undistort points
    mat = mat.reshape(2);
    cv::undistortPoints(mat, mat, static_cast<Pinhole *>(mpCamera)->toK(), mDistCoef, cv::Mat(),
                        mK);
    mat = mat.reshape(1);
