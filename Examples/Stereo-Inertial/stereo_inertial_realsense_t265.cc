...
int main(int argc, char** argv) {
  ...
  auto imu_callback = [&](const rs2::frame& frame) {
    ...
    if (rs2::frameset fs = frame.as<rs2::frameset>()) {
      ...
      imCV = cv::Mat(cv::Size(width_img, height_img), CV_8U, (void*)(color_frame.get_data()),
                     cv::Mat::AUTO_STEP);
      imCV_right = cv::Mat(cv::Size(width_img, height_img), CV_8U,
...
#  endif
    ...
    for (int i = 0; i < vGyro.size(); ++i) {
      ...
      if (isnan(vAccel[i].x) || isnan(vAccel[i].y) || isnan(vAccel[i].z) || isnan(vGyro[i].x)
          || isnan(vGyro[i].y) || isnan(vGyro[i].z) || isnan(vGyro_times[i])) {
        exit(-1);
      }
    }
