...
namespace ORB_SLAM3 {
  ...
  namespace IMU {
    ...
    public:
      Calib(const Sophus::SE3<float> &Tbc, const float &ng, const float &na, const float &ngw,
            const float &naw) {
      ...
      // void Set(const cv::Mat &cvTbc, const float &ng, const float &na, const float &ngw, const
      // float &naw);
