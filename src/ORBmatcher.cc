...
namespace ORB_SLAM3 {
  ...
  int ORBmatcher::Fuse(KeyFrame *pKF, Sophus::Sim3f &Scw, const vector<MapPoint *> &vpPoints,
      ...
      if (pMP) {
        if (!LastFrame.mvbOutlier[i]) {
          ...
          if (CurrentFrame.Nleft != -1) {
            ...
            if (bForward)
              vIndices2
                  = CurrentFrame.GetFeaturesInArea(uv(0), uv(1), radius, nLastOctave, -1, true);
            else if (bBackward)
              vIndices2
                  = CurrentFrame.GetFeaturesInArea(uv(0), uv(1), radius, 0, nLastOctave, true);
            else
              vIndices2 = CurrentFrame.GetFeaturesInArea(uv(0), uv(1), radius, nLastOctave - 1,
                                                         nLastOctave + 1, true);
